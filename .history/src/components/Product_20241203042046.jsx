...
const Product = ({ product }) => {
  return (
    <div className="relative bg-white shadow-lg rounded-lg p-4 max-w-sm h-10 w-10">
      {/* Sale Badge */}
      {product.onSale && (
        <span className="absolute top-2 left-2 bg-white border rounded-lg text-green-900 text-xs font-bold px-2 py-1 ">
          Sale!
        </span>
      )}
      ...
      {/* Add to Cart Button */}
      <button className="w-full bg-amber-900 text-white py-2 rounded-md hover:bg-amber-950 transition">
        Add to Cart
      </button>
    </div>
  );
